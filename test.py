...
parser.add_argument('--root', default='/your path to the data folder(i.e. containing BraTS2021_ValidationData)/', type=str)
...
def main():
    ...
    valid_root = os.path.join(arg.root, args.valid_dir)
    valid_set = BraTS(valid_list, valid_root, mode='test')
    print('Samples for valid = {}'.format(len(valid_set)))

    batch_size = 8
    num_gpu = (len(args.gpu)+1) // 2
    valid_loader = DataLoader(valid_set, batch_size=batch_size // num_gpu, shuffle=False, num_workers=args.num_workers, pin_memory=True)
