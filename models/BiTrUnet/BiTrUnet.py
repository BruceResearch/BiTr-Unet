#Modified from https://github.com/Wenxuan-1119/TransBTS
import torch
import torch.nn as nn
from models.BiTrUnet.Transformer import TransformerModel
from models.BiTrUnet.PositionalEncoding import FixedPositionalEncoding,LearnedPositionalEncoding,LearnedPositionalEncoding_4
from models.BiTrUnet.Unet_skipconnection import Unet
...
class BiTransUnet(BiTransformerUnet):
    def __init__(
        ...
        self.Enblock7_1 = EnBlock1_4(in_channels=self.embedding_dim_4 // 2) # 256
        self.Enblock7_2 = EnBlock2_4(in_channels=self.embedding_dim_4 // 4) # 128
        
        self.Enblock8_1 = EnBlock1(in_channels=self.embedding_dim) #256
        self.Enblock8_2 = EnBlock2(in_channels=self.embedding_dim // 2) #128
        
        self.DeUp5 = DeUp_Cat(in_channels=self.embedding_dim //2 , out_channels=self.embedding_dim//2) #128
        self.DeBlock5 = DeBlock(in_channels=self.embedding_dim //2)#128

        self.Cat4 = Cat(in_channels=self.embedding_dim_4//4, out_channels=self.embedding_dim_4//4)
        self.Block4 = DeBlock(in_channels=self.embedding_dim_4//4)
        
        self.DeUp4 = DeUp_Cat(in_channels=self.embedding_dim_4//4, out_channels=self.embedding_dim_4//8) #64
        self.DeBlock4 = DeBlock(in_channels=self.embedding_dim_4//8)

        self.DeUp3 = DeUp_Cat(in_channels=self.embedding_dim_4//8, out_channels=self.embedding_dim_4//16) #32
        self.DeBlock3 = DeBlock(in_channels=self.embedding_dim_4//16)

        self.DeUp2 = DeUp_Cat(in_channels=self.embedding_dim_4//16, out_channels=self.embedding_dim_4//32)#16
        self.DeBlock2 = DeBlock(in_channels=self.embedding_dim_4//32)

        self.endconv = nn.Conv3d(self.embedding_dim_4 // 32, 4, kernel_size=1) #16

        #self.decoder8_1 = UBlock(self.embedding_dim, self.embedding_dim // 2,  self.embedding_dim // 4, norm_layer, dropout=dropout)
        #self.decoder8_2 = UBlock(self.embedding_dim // 4, self.embedding_dim // 4,  self.embedding_dim // 4, norm_layer, dropout=dropout)

                
    def decode(self, x1_1, x2_1, x3_1, x4_1, x, intmd_x, x4_t, intmd_x4_t, intmd_layers=[1, 2, 3, 4], intmd_layers_4=[1, 2, 3, 4]):
        ...
        encoder_outputs_4 = {} #
        all_keys_4= [] #
        for i in intmd_layers_4: #
            val_4 = str(2 * i - 1) #
            _key_4 = 'Z' + str(i) #
            all_keys_4.append(_key_4) #
            encoder_outputs_4[_key_4] = intmd_x4_t[val_4] #
        all_keys_4.reverse() #

        x7 = encoder_outputs_4[all_keys_4[0]] #
        x7 = self._reshape_output_4(x7) # 2 256 16 16 16 
        x7 = self.Enblock7_1(x7) #
        x7 = self.Enblock7_2(x7) # 2 128 32 32 32 
        ...
        y5 = self.DeUp5(x8, x4_1)  # (1, 128, 32, 32, 32)
        y5 = self.DeBlock5(y5)

        y4 = self.Cat4(x7, y5) # (1, 64, 32, 32, 32)
        y4 = self.Block4(y4)
        
        y4 = self.DeUp4(y4, x3_1)  # (1, 64, 32, 32, 32)
        y4 = self.DeBlock4(y4)
...
class EnBlock1(nn.Module):
    def __init__(self, in_channels):
        ...
        self.bn1 = nn.BatchNorm3d(256 // 2)
        self.relu1 = nn.ReLU(inplace=True)
        self.bn2 = nn.BatchNorm3d(256 // 2)
...
class EnBlock1_4(nn.Module):
    def __init__(self, in_channels):
        super(EnBlock1_4, self).__init__()
...
class EnBlock2_4(nn.Module):
    def __init__(self, in_channels):
        super(EnBlock2_4, self).__init__()
